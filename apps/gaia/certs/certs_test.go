...
// TestGenerateCA_ErrorCases tests error handling in CA generation
func TestGenerateCA_ErrorCases(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			commonName:   "",
			validityDays: 365,
			expectError:  false, // Empty CN is technically valid
		},
		{
			name:         "ZeroValidityDays",
			commonName:   "Test CA",
			validityDays: 0,
			expectError:  false, // Will create cert valid for 0 days * 10 = 0 days
		},
		{
			name:         "NegativeValidityDays",
			commonName:   "Test CA",
			validityDays: -1,
			expectError:  false, // Will create cert with expiry in the past
		},
		{
			name:         "VeryLongCommonName",
			commonName:   strings.Repeat("A", 1000),
			validityDays: 365,
			expectError:  false, // Long CN should still work
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, _, err := generateCA(tt.commonName, tt.validityDays)
			if (err != nil) != tt.expectError {
				t.Errorf("generateCA() error = %v, expectError %v", err, tt.expectError)
			}
		})
	}
}

// TestGenerateCert_ErrorCases tests error handling in certificate generation
func TestGenerateCert_ErrorCases(t *testing.T) {
	// Generate CA first
	caKey, caCert, err := generateCA("Test CA", 365)
	if err != nil {
		t.Fatalf("Failed to generate CA: %v", err)
	...
	tests := []struct {
		...
		{
			name:         "SpecialCharactersInCN",
			commonName:   "test@#$%^&*()client",
			isServer:     false,
			validityDays: 365,
		},
		{
			name:         "ServerCertZeroValidity",
			commonName:   "server",
			isServer:     true,
			validityDays: 0,
		},
		{
			name:         "ClientCertNegativeValidity",
			commonName:   "client",
			isServer:     false,
			validityDays: -1,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			key, cert, err := generateCert(tt.commonName, caKey, caCert, tt.isServer, tt.validityDays)
			if err != nil {
				t.Errorf("generateCert() unexpected error = %v", err)
				return
			}
			if key == nil || cert == nil {
				t.Error("generateCert() returned nil key or cert")
			}
		})
	}
}

// TestSaveCert_ErrorCases tests error handling in certificate saving
func TestSaveCert_ErrorCases(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupFunc: func() string {
				return "/nonexistent/directory/cert.crt"
			},
			expectError: true,
		},
		{
			...
			setupFunc: func() string {
				...
				os.Mkdir(roDir, 0555)
				return filepath.Join(roDir, "cert.crt")
			},
			expectError: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := saveCert(certPath, caCert)
			if (err != nil) != tt.expectError {
				t.Errorf("saveCert() error = %v, expectError %v", err, tt.expectError)
			}
		})
	}
...
// TestSaveKey_ErrorCases tests error handling in key saving
func TestSaveKey_ErrorCases(t *testing.T) {
	// Generate test key
	caKey, _, err := generateCA("Test CA", 365)
	if err != nil {
		t.Fatalf("Failed to generate CA: %v", err)
	...
	tests := []struct {
	...
	}{
		{
			name: "InvalidDirectory",
			setupFunc: func() string {
				return "/nonexistent/directory/key.key"
			},
			expectError: true,
		},
		{
			...
			setupFunc: func() string {
				...
				roDir := filepath.Join(tempDir, "readonly")
				os.Mkdir(roDir, 0555)
				return filepath.Join(roDir, "key.key")
			},
			expectError: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := saveKey(keyPath, caKey)
			if (err != nil) != tt.expectError {
				t.Errorf("saveKey() error = %v, expectError %v", err, tt.expectError)
			}
		})
	}
}

// TestLoadCA_InvalidData tests loading corrupted certificate data
func TestLoadCA_InvalidData(t *testing.T) {
	tests := []struct {
		name          string
	...
	}{
		{
			...
			setupFunc: func(t *testing.T) (string, string) {
				...
				// Generate valid key
				caKey, _, _ := generateCA("Test CA", 365)
				saveKey(keyPath, caKey)

				return certPath, keyPath
			},
			errorContains: "failed to decode CA certificate PEM",
		},
		{
			name: "InvalidKeyPEM",
			setupFunc: func(t *testing.T) (string, string) {
				...
				// Generate valid cert
				_, caCert, _ := generateCA("Test CA", 365)
				saveCert(certPath, caCert)

				// Write invalid key PEM
				os.WriteFile(keyPath, []byte("not a valid key PEM"), 0600)

				return certPath, keyPath
			},
			errorContains: "failed to decode CA key PEM",
		},
		{
			name: "InvalidCertData",
			setupFunc: func(t *testing.T) (string, string) {
				tempDir := t.TempDir()
				certPath := filepath.Join(tempDir, "cert.crt")
				keyPath := filepath.Join(tempDir, "key.key")

				// Write PEM with invalid certificate data
				invalidCertPEM := pem.EncodeToMemory(&pem.Block{
					Type:  "CERTIFICATE",
					Bytes: []byte("invalid certificate data"),
				})
				os.WriteFile(certPath, invalidCertPEM, 0644)
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (string, string) {
				...
				os.WriteFile(keyPath, invalidKeyPEM, 0600)

				return certPath, keyPath
			},
			errorContains: "failed to parse CA private key",
		},
	}
...
// TestGenerateCA_Integration_ErrorCases tests error handling in GenerateCA
func TestGenerateCA_Integration_ErrorCases(t *testing.T) {
	tests := []struct {
		name          string
		...
		{
			name: "InvalidCertsDirectory",
			setupFunc: func() *config.Config {
				return &config.Config{
					CertsDirectory: "/nonexistent/directory",
					CACertFile:     "ca.crt",
					CertExpiryDays: 365,
				}
			},
		...
		},
		{
			...
			setupFunc: func() *config.Config {
				...
				os.Mkdir(roDir, 0555)
				return &config.Config{
					CertsDirectory: roDir,
					CACertFile:     "ca.crt",
					CertExpiryDays: 365,
				}
			},
			errorContains: "failed to save CA certificate",
		},
	}
...
// TestGenerateServerCertificate_ErrorCases tests error handling in GenerateServerCertificate
func TestGenerateServerCertificate_ErrorCases(t *testing.T) {
	// Test missing CA
	t.Run("MissingCA", func(t *testing.T) {
		...
		err := GenerateServerCertificate(cfg, "test-server")
		if err == nil {
			t.Fatal("Expected error when CA doesn't exist")
		}
		if !strings.Contains(err.Error(), "failed to read CA certificate file") {
			t.Errorf("Expected error about missing CA, got: %v", err)
		}
	})
...
// TestGenerateClientCertificate_ErrorCases tests error handling in GenerateClientCertificate
func TestGenerateClientCertificate_ErrorCases(t *testing.T) {
	// Test missing CA
	t.Run("MissingCA", func(t *testing.T) {
		tempDir := t.TempDir()
		cfg := &config.Config{
		...
		err := GenerateClientCertificate(cfg, "test-client")
		if err == nil {
			t.Fatal("Expected error when CA doesn't exist")
		}
		if !strings.Contains(err.Error(), "failed to read CA certificate file") {
...
// TestGenerateClientCertificateData_EdgeCases tests edge cases
func TestGenerateClientCertificateData_EdgeCases(t *testing.T) {
	// Generate CA
	caKey, caCert, err := generateCA("Test CA", 365)
	if err != nil {
		t.Fatalf("Failed to generate CA: %v", err)
	...
	tests := []struct {
		...
		{
			name:         "VeryLongClientName",
			clientName:   strings.Repeat("client", 100),
			validityDays: 365,
		},
		{
			name:         "SpecialCharacters",
			clientName:   "client-with-特殊字符",
			validityDays: 365,
		},
		{
			name:         "ShortValidity",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			certPEM, keyPEM, err := GenerateClientCertificateData(tt.clientName, caCert, caKey, tt.validityDays)
			if err != nil {
				t.Fatalf("GenerateClientCertificateData() error = %v", err)
			}
